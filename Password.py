...
class Password:
    ...
    def check_assertion(self) -> AssertionError:
        if self.lowercase_number + self.uppercase_number + self.digit_number + self.symbol_number > self.n:
            raise AssertionError("The length of the password must be greater or equal to the number of desired characters")
...
# Test 1.1: check_assertion
# p = Password("a", 2, 2, 2, 2)
# print(p.generate())

# Test 1.2: check_assertion
...
# Test 3: generate (and therefore: check_numbers), with self.lowercase_number + self.uppercase_number + self.digit_number + self.symbol_number == self.n
